...
PassportConfigurator.prototype.configureProvider = function (name, options) {
	  ...
	  if (user) {
	      ...
	      profileToUser: function(provider, profile){
	        // returns a user object
                var mail = [].concat(profile[ldap_attribute_for_mail])[0]
                var username = [].concat(profile[ldap_attribute_for_username])[0]
		return {
		  username: username,
		  password: "etisurnaotpdanets",
		  email: mail
		}
	      }
	    },options)
            options_for_creation.autoLogin = true
	    self.userIdentityModel.login(name, authScheme, user, {}, 
  ...
  if (authType === 'local') {
    ...
    var ldapCallback = options.customCallback || function (req, res, next) {
